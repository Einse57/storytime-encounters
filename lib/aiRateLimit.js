 ...
 * @param {{ maxPerDay: number, maxPerMinute: number, softMessage: string, softMinuteMessage?: string }} limits
...
export function checkAiRateLimit(key, limits) {
  ...
  if (bucket.minuteCount >= limits.maxPerMinute) {
    ...
    return {
      ...
      error:
        limits.softMinuteMessage ||
        'A few AI requests at once — wait a moment and try again.',
    };
  }
...
export const ILLUSTRATE_LIMITS = {
  ...
  maxPerMinute: 2,
  softMessage:
    'App daily AI paint limit reached (20/day) — try again tomorrow, or use Copy Prompt.',
  softMinuteMessage:
    'App minute AI paint limit (2/min) — wait a moment and try again.',
};

// Long Enhance sessions send several parts back-to-back; keep the minute
// budget high enough that a normal 20–30 min story does not die on part 4.
