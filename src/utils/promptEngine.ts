...
/**
 * One hero illustration prompt from the full story transcript + style.
 * Used after Build/Rebuild so we only burn a single Gemini image gen.
 */
export const buildHeroImagePrompt = (
  style: ComicStyle,
  transcript: string,
  ...
  const storyBlob =
    ...
    (panels && panels.length > 0
      ? panels.map((p) => p.caption).filter(Boolean).join(' ')
      : '') ||
    [seed?.setting, seed?.conflict, seed?.hook].filter(Boolean).join('. ');

  const storyMoment = (storyBlob || 'an unfolding tabletop story adventure')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 1200);

  return `${preset.promptPrefix} A single wide hero illustration capturing the whole story arc set in ${settingText}. Depict the essence of this adventure in one cohesive painting (not a multi-panel comic page): ${storyMoment} ${preset.promptSuffix}`;
};
