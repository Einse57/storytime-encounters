...
export function buildScenesFromSession(
  ...
  if (chunks.length === 0) {
    return {
      ...
      note: 'Story Log is empty, so Rebuild only used seeds and sparks. Old art cleared — painting all scenes into one storyboard. Add what you said or typed, then Rebuild again for a fuller story.',
    };
  }
  ...
  return {
    panels,
    note: `Rebuilt ${panels.length} scenes from the story log. Old art cleared — painting all scenes into one storyboard.`,
  };
}
